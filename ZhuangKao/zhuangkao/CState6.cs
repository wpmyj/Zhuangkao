...
namespace zhuangkao
{
    public class CState6:CState
    {
        ...
        {
            tm.Stop();
            if (eform.cm.CurrData.xian[7] == 1)//判断离开8线
            {
                StateManager.SwitchState("State7");
                eform.toolStripStatusLabel1.Text = "状态：7";
                ...
                try
                {
                    eform.timer1.Interval = eform.StateTime[6] * 1000 / che.StepCount;//根据配置计算状态5在设定时间内的速度（屏幕刷新速度）
                }
                catch
                {
        ...
        public override void Xian8_Bump()
        {
            ...
            lkxian7 = true;//代表离开了线8，如果再次压线7则代表进入下一个状态
            base.Xian8_Bump();
        }
