...
namespace zkcenter
{
    ...
    public class CNetData
    {
        ...
        public CNetData(byte[] data)  //第0-3位为命令编号，第4-7位为设备编号，8-11位为证件编号位数,第5以后的位为证件编号，
        {
            ...
            //取出设备编号
            this.intDevnum  = BitConverter.ToInt32(data, 4);

            //取出证件号码长度
            this._intZjnum = BitConverter.ToInt32(data, 8);

            //取出证件号码
            if (_intZjnum > 0)
            {
                this.strZjbh = Encoding.UTF8.GetString(data, 12, _intZjnum);
            }
            else
            {
                this.strZjbh = null;
            ...
            //取出姓名长度
            this._intXmnum = BitConverter.ToInt32(data, 12+_intZjnum);
            if (_intXmnum > 0)
            {
                this.Xm = System.Text.Encoding.Default.GetString(data, 16 + _intZjnum, _intXmnum);
            }else
            {
                this.Xm=null;
            }
