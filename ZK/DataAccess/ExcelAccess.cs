...
{   
    ...
    {   
        ...
        public DataSet ImportFromExcel(string strTableName)   
        {   
            DataSet ds = new DataSet();   
            if (openFileDlg.ShowDialog() == System.Windows.Forms.DialogResult.OK)   
                ds = doImport(openFileDlg.FileName, strTableName);   
            return ds;   
        }   
        ...
        /// <returns></returns>   
        public DataSet ImportFromExcel(string strFileName, string strTableName)   
        {   
            DataSet ds = new DataSet();   
            ds = doImport(strFileName, strTableName);   
            return ds;   
        }   
        ...
        /// <returns>DataSet</returns>   
        private DataSet doImport(string strFileName, string strTableName)   
        {   
            if (strFileName == "") return null;   
            strConn = "Provider=Microsoft.Jet.OLEDB.4.0;" + "Data Source=" + strFileName + ";" + "Extended Properties=Excel 8.0;";   
            OleDbDataAdapter ExcelDA = new OleDbDataAdapter("SELECT * FROM " + strTableName, strConn);   
